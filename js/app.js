...
const App = (() => {
  ...
  function initVoice(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){el.mic.disabled=true;el.mic.title='此浏览器不支持语音输入';return}recog=new SR();recog.lang='es-ES';recog.interimResults=false;recog.maxAlternatives=1;recog.onresult=e=>{el.input.value=e.results[0][0].transcript;state.inputMode='voice';submit()};recog.onend=()=>el.mic.classList.remove('listening');recog.onerror=()=>el.mic.classList.remove('listening')}
  function toggleVoice(){if(!recog||state.answered)return;state.inputMode='voice';el.mic.classList.add('listening');try{recog.start()}catch{}}
